...
*Status (2026-09-13, Phase B WS-B2):* Resolved by decision, per BEHAVIOR_RECIPES_PLAN D1: `min_avg_forward_vel = 2.0` is retired from `configs/trex/behavior.toml` — closing speed is measured (`avg_forward_vel` in the summary), not gated — and the stage advances on `task_success/v1` instead (see SS2). The committed 2026-03 row's 1.68 m/s would have failed the retired target; it stays a historical ladder result.
...
*Status (2026-09-13, Phase B WS-B2):* Fixed as suggested in `configs/trex/behavior.toml`. The hunting statue was measured with provenance — **602.13 ± 175.35 over 40 episodes, 40/40 full horizon** (episode length 1000.0 of 1000, all truncated), `python environments/shared/scripts/zero_action_baseline.py trex:3 --episodes 40 --seed 3042`, mujoco 3.10.0, physics revision 7, reset noise 0.05 — which supersedes the unprovenanced 557 ± 142 (n = 10) figure quoted above. The absolute `collapse_peak_floor = 100.0` (6× below the statue) is removed, not shadowed, for `collapse_peak_floor_reference = 602.0` / `collapse_peak_floor_fraction = 0.45` (floor 270.9) with `statue_constants_physics_revision = 7`, so `test_statue_constant_freshness.py` now guards the stage; `min_avg_reward` becomes the collapse rail at 361 = round(0.6 × 602.13) (decision D-B4); `collapse_min_evals / collapse_patience / collapse_drop_fraction` are explicit at 20 / 10 / 0.5; `collapse_peak_warmup_timesteps = 1,000,000` is a judgment (decision D-B5) pending a replay of the first full hunting run's `evaluations.npz` (KNOWN_ISSUES follow-up).
...
*Status (2026-09-13, Phase B WS-B1/WS-B2):* Fixed as suggested. The `task_success/v1` gate kind (`environments/shared/curriculum/task_success_gate.py`) certifies the exact one-sided 95% Clopper-Pearson lower bound (`recovery_gate.binomial_lcb`) on the selected checkpoint's per-episode `evaluation_selected.csv`, hash-bound to the handoff checkpoint, at the declared `min_eval_episodes = 30` (decision D-B1) against `min_success_lcb = 0.5` (decision D-B2, PROVISIONAL until the first Phase-B pilot re-freezes it): 20/30 clears at 0.5006, 19/30 does not (0.4669); at n = 40, 26/40 (0.5081) versus 25/40 (0.4828). The committed 2026-03 result recomputes to 29/30 → LCB 0.8514 from its rounded mean — a recomputation, not a re-judgement, since the row holds no per-episode evidence and was judged at `min_success_rate` 0.25 with no velocity term. The in-training consecutive-3 counter stays as scheduler hysteresis only (decision D-B3); the certifying verdict is one post-stage panel, re-derived at publication from the same rows. n was kept at 30 rather than sized up (D-B1), so the bar separates ≥ 0.67 from 0.5, not 0.45 from 0.60.
