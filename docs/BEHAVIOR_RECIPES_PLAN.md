...
| D-D19 | Taken 2026-09-26 (the cleanup plan's decision 3): the CHANGELOG release is cut before PR-A, so the retirement's Removed and Migration entries head a short section. The three undated `[Unreleased]` headings (v0.3.8, v0.3.2, v0.3.0) are dated, an empty `[Unreleased]` opens above them, `pyproject.toml`'s version moves from `0.3.8.dev0` to `0.3.9.dev0`, and the maintainer tags `v0.3.8` on the commit that dates its section; v0.4.0 stays ROADMAP's "Into the Wild" milestone. The version is provenance (`stage_config.json`'s `library_version`, the HPT metrics payload, `provenance.json`'s `dependency_versions`), so a resume after the bump records environment drift, as a new repository commit already does. |
| D-D20 | Taken 2026-09-26 (the cleanup plan's decision 7, option (a); carried out by its CU-3): a Colab reclaim may not leave the final pair or a handoff pair truncated, nor a new file beside the previous pair's other half. On a Drive/GCS mount `train()` saves the final pair and the best and robust-best handoff pairs to local scratch and publishes them into `models/`, as it already did the periodic pairs; because these names are reused, the destination file published last is removed before anything is published, so a reclaim part-way leaves at worst a handoff sidecar without its zip (which `select_handoff_checkpoint` skips) or a final zip without its sidecar (which `checkpoint_pair_problem` reports, so D-D16's RESUME and chain-loop rules apply unchanged). Off a mount the pairs are written in place, as before. File names and bytes are unchanged, so no digest moves. As carried out by CU-3: `curriculum.publish_staged_pair` does the publishing, `RobustBestModelCallback` takes a `staging_dir` and `SaveVecNormalizeCallback` a `publish_dir`, and `_save_final_and_sync_tb` stages the final pair. Unblocks PR-13, whose evidence writer follows it by writing through `file_io`'s atomic helpers. |
