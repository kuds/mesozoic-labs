...
| [BEHAVIOR_RECIPES_PLAN.md](BEHAVIOR_RECIPES_PLAN.md) | 2026-09-05 | Active — adopted design turning the stage ladder into a DAG of behavior recipes (stand / walk / hunt / follow direction), each a separately certified, separately published policy. Phase A (manifest v2, per-node gate verdicts, cross-run ancestor reuse, per-deliverable publication, the notebook chain loop) merged 2026-09-12 in #528–#530; Phase B (the measured hunting gate `task_success/v1`, gate-configuration digests with reuse rule 7, seed replication as provenance) merged 2026-09-13 in #533, #534 (hunting gate) and #535 (seed replication); Phase C (the 3-dim command segment on all six species, `widen_checkpoint`, the notebook `WIDEN_FROM` knob) implemented 2026-09-13/14, its maintainer sessions (widen + re-panel the certified stance, the recovery re-roll, the C½ walker) pending against the template note; §6.1 records decisions D-A1–D-A24, D-B1–D-B17 and D-C1–D-C16; Phases D–E (follow leaf) pending |
...
| [TREX_STANCE_WIDENED_INTERFACE_2026_09.md](investigations/TREX_STANCE_WIDENED_INTERFACE_2026_09.md) | 2026-09-14 | **(template — pending maintainer sessions)** The certified stance parents `20260810_145546` / `20260815_205206` widened to the Phase C interface (policy interface r13) with `widen_checkpoint` + `WIDEN_FROM`, re-paneled under the new task hash, one recovery freeze re-rolled from the widened handoff, and the C½ walker; the sessions' exact knobs and commands are written, every result is a placeholder; both parents are r11 archives, two revisions behind r13, so the two widen sessions set `WIDEN_MAX_REVISION_GAP = 2` (decision D-C17; KNOWN_ISSUES) |
