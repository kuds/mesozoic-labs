...
2. **Order.** CU-1 (mypy with SB3, readable CI logs; D-D18) comes first, then the CHANGELOG release cut (D-D19). Next
   is PR-A (Ray Tune, the Vertex AI tuning sweeps and mjlab, and since D-D17 was taken on 2026-09-26 the single-job
...
1. **CU-1 (CI signal), before PR-A.** CU-1, PR-A and PR-B all edit `python-ci.yml`. PR-A changes the `test-sb3`
   ...
   **Carried out by the CU-1 PR (2026-09-26, D-D18):** the SB3 job's new mypy step reports no errors, from 23 in
   `test-sb3`'s environment and 21 locally (§3.2).
2. **The release cut (D-D19), then PR-A (§4.5), then PR-B (§4.6).** The release PR dates the three `[Unreleased]`
   headings, opens an empty one and moves the version to `0.3.9.dev0`; the maintainer tags `v0.3.8` on the commit that
   dates its section. PR-B depends on PR-A's D-D17 and tag (§4.7). Both use the digest-snapshot
...
| Pre-commit pins ruff 0.4.4 while CI installs the latest ruff (Testing / CI, LOW) | CU-1 | CU-1 (deleted by it, 2026-09-26) |
| CI's mypy never sees SB3 or torch types (Testing / CI, LOW) | Decision 8 | CU-1 (deleted by it, 2026-09-26) |
...
**Widened by D-D17 (2026-09-26).** The maintainer retired the single-job Vertex AI route and GCS artifact upload as
well, so PR-A also deletes `scripts/setup_vertex_ai.sh`, the `Dockerfile`, `.dockerignore` and
...
`reporting/csv_output.py`, and the `[gcp]` extra. The counts above predate that; PR-A
re-derives the whole list and its line counts when it opens. The bullets below that keep the route or `[gcp]` are
superseded where marked. `tb_sync.py` stays (§4.9) even though its `/gcs/` branch served the Vertex FUSE mount.
...
- **`pyproject.toml`.** Remove `cloudml-hypertune` (:41), `[ray]` (:56-61), `[mjlab]` (:62-67) and the mjlab coverage
  ...
  `curriculum --gcs-bucket` needs `google-cloud-storage` (`config.py:881,952`). *Superseded by D-D17:* `[gcp]` goes
  with the GCS upload code, so `[all]` becomes `[train,jax,viz,dev]`.
...
**Draft D-D17** (PR-B fills in `<…>` with an appended "Recorded:" sentence). *Recorded 2026-09-26:* the maintainer took
D-D17 with the single-job route and GCS upload retired too; the row is in
[BEHAVIOR_RECIPES_PLAN.md](BEHAVIOR_RECIPES_PLAN.md) §6.2, and PR-A and PR-B append their tag SHAs to it rather than
filling in this draft, which stays as proposed:
...
**Risks (PR-A):**
- Dropping `gcp` from `[all]` would silently disable GCS upload; the only sign is a warning. (Under D-D17 the upload
  code leaves with `[gcp]`, so a `--gcs-bucket` caller gets an argparse error instead.)
...
- Delete the `jax` (:43-48) and `jax-cpu` (:49-55) extras; `[all]` becomes `[train,viz,gcp,dev]` (`[train,viz,dev]`
  under D-D17, since PR-A removes `[gcp]`).
...
- **Removing anything §4.9 keeps, dropping `gcp` from `[all]` or the Drive summary's sweep branch, or reusing the
  ...
  PR-A together with the upload code, never on its own.)
