...
byte-identical at `f850815` and `be63a58`. Line numbers drift with every merge, so re-read before editing.
...
2. **Order.** CU-1 (mypy with SB3, readable CI logs) comes first. Next is PR-A (Ray Tune, the Vertex AI tuning sweeps
   and mjlab, with proposed D-D17; its acceptance runs the digest-snapshot harness this plan adds, §4.4), then PR-B
...
9. **Before PR-A merges,** the maintainer records D-D17, settles whether the single-job Vertex route stays, and tags
   the archive point. The digest-snapshot harness is already in the repository (added with this plan, 2026-09-26;
   §4.4). The frozen-core reference copies are not: PR-B rebuilds them from `f850815` (§4.2).
...
D-D17 is reserved for the retirement (**proposed D-D17**), because PR-A's text must cite it. The other rows take D-D18
onward, in the order taken, even if one is taken before PR-A merges. Record every row append-only in
...
2. **PR-A (§4.5), then PR-B (§4.6).** PR-B depends on PR-A's D-D17 and tag (§4.7). Both use the digest-snapshot
...
| PR-13 terrain_command gate | Yes, to certify terrain | CU-3; decisions 6 and 12 | Invariant 10's new case covers only the SB3 manager. Land PR-B first, because it deletes `test_gate_dispatch_fail_closed.py:161-351` |
...
| The best and robust-best handoff pairs are written straight to the mount (Training / RL, MEDIUM) | Decision 7 | CU-3 |
...
| Pre-commit pins ruff 0.4.4 while CI installs the latest ruff (Testing / CI, LOW) | CU-1 | CU-1 |
| CI's mypy never sees SB3 or torch types (Testing / CI, LOW) | Decision 8 | CU-1 |
...
- **CI (`python-ci.yml`).** :267 becomes `".[train,test,viz]"`. :269-306 shrink to `import stable_baselines3, torch`
...
- **`pyproject.toml`.** Remove `cloudml-hypertune` (:41), `[ray]` (:56-61), `[mjlab]` (:62-67) and the mjlab coverage
  ...
  `curriculum --gcs-bucket` needs `google-cloud-storage` (`config.py:881,952`).
- **Vertex single-job route** (if decision 1 keeps it). Drop `setup_vertex_ai.sh:183`. Delete `vertex-ai.md:570-679`
...
**Draft D-D17** (PR-B fills in `<…>` with an appended "Recorded:" sentence):
...
**Risks (PR-A):**
- Dropping `gcp` from `[all]` would silently disable GCS upload; the only sign is a warning.
...
- Delete the `jax` (:43-48) and `jax-cpu` (:49-55) extras; `[all]` becomes `[train,viz,gcp,dev]`.
...
amendment cite, and the first tag. The harness, PR-B's acceptance step 1, is already in the repository (§4.4). The
...
- **Removing anything §4.9 keeps, dropping `gcp` from `[all]` or the Drive summary's sweep branch, or reusing the
  ...
  into `sweeps/`. The `JAX` branch is not an ancestor of `main`.
