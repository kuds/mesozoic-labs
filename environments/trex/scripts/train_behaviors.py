"""Train/evaluate opt-in direction and gentle-terrain PPO pilots.

Example: python -m environments.trex.scripts.train_behaviors --help
...
def read_recipe(path: Path) -> tuple[dict[str, Any], DirectionCommandConfig, TerrainConfig | None, dict[str, Any]]:
    """Load a bounded pilot recipe, rejecting misspelled configuration keys."""
    with path.open("rb") as stream:
        recipe = tomllib.load(stream)
    unknown = set(recipe) - {"pilot", "commands", "terrain", "env", "ppo"}
    if unknown:
    ...
    allowed_env = set(inspect.signature(TRexEnv.__init__).parameters) | set(
        inspect.signature(TRexBehaviorEnv.__init__).parameters
    )
    ...
    for section, key, default in (("pilot", "timesteps", 3_000_000), ("ppo", "warmup_timesteps", 100_000)):
    ...
    baseline = Path(__file__).parents[3] / "configs" / "trex" / "locomotion.toml"
...
def main(argv: list[str] | None = None) -> None:
    ...
    steps = args.steps if args.steps is not None else int(recipe.get("pilot", {}).get("timesteps", 3_000_000))
    if steps < 0:
        parser.error("pilot.timesteps must be nonnegative")
    ...
    model, normalizer, preparation = loader(
        args.checkpoint, args.vecnormalize, wrapped, learning_rate=learning_rate, behavior_identity=behavior_identity
    )
    ...
    if args.resume and args.steps is None:
        steps = max(0, int(recipe["pilot"]["timesteps"]) - (model.num_timesteps - stage_start))
    ...
    git = subprocess.run(["git", "rev-parse", "HEAD"], cwd=Path(__file__).parents[3], text=True, capture_output=True)
    dirty = subprocess.run(
        ["git", "status", "--porcelain"], cwd=Path(__file__).parents[3], text=True, capture_output=True
    )
