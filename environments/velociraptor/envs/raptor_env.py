...
class RaptorEnv(gym.Env):
    ...
    def _get_reward_info(self, action: np.ndarray) -> Tuple[float, Dict[str, float]]:
        ...
        # Sensor indices for tail_tip_vel (framepos is 3 values, framevelocity is 6: 3 lin + 3 ang)
        tail_tip_vel = self.data.site_xvelp[self.tail_tip_site_id]  # Linear velocity
        tail_tip_angvel = self.data.site_xvelr[self.tail_tip_site_id]  # Angular velocity
