...
class RaptorEnv(BaseDinoEnv):
    ...
    def _cache_ids(self):
        ...
        self._body_ground_geoms = {
            self.torso_geom_id, self.neck_geom_id, self.head_geom_id,
    ...
    def _is_terminated(self) -> Tuple[bool, Dict[str, Any]]:
        ...
        _tail_geoms = {self.tail_3_geom_id, self.tail_4_geom_id, self.tail_5_geom_id}
        for i in range(self.data.ncon):
            contact = self.data.contact[i]
            geom1, geom2 = contact.geom1, contact.geom2
            ...
            if floor_contact_geom is not None:
                if floor_contact_geom in _tail_geoms:
