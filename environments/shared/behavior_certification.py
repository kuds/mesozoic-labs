"""Evidence-backed publication and paired comparisons for command/terrain skills.
...
def _evaluation_contract() -> dict[str, Any]:
    versions = {}
    for package in ("mujoco", "numpy", "stable_baselines3", "torch"):
        try:
    ...
    return {
        "sources": {
            ...
            for name in ("behavior_certification.py", "behavior_evaluation.py", "behavior_checkpoint.py")
        },
        "runtime": versions,
    }


def behavior_library_key(species: str, behavior: str, identity: Mapping[str, Any]) -> dict[str, Any]:
    rules = load_certification_rules()
    gate = {key: value for key, value in rules.items() if key != "comparison"}
    return {
        "species": species,
        "algorithm": "ppo",
        "behavior": behavior,
        "plant_identity": identity["parent_plant"],
        "task_sha256": _digest(identity),
        "gate_sha256": _digest({"rules": gate, "evaluation": _evaluation_contract()}),
    }
...
def comparison_from_panel(report: Mapping[str, Any], identity: Mapping[str, Any]) -> dict[str, Any]:
    rules = load_certification_rules()["comparison"]
    episodes = report["episodes"]
    measured = [episode_measurements(episode, float(identity["course_distance"])) for episode in episodes]
    families = report["terrain_coverage"]["enabled_families"]
    metrics = []
    for name, margin_key in (
        ("survival", "survival_margin"),
        ("tracking", "tracking_margin"),
        ("settling", "settling_margin"),
        ("stopping", "settling_margin"),
        ("turning", "settling_margin"),
        ("progress", "progress_margin"),
    ):
        if name == "stopping" and not identity["commands"]["stop_probability"]:
            continue
        if name == "turning" and not identity["commands"]["turn_increment_max"]:
            continue
        # Every terrain is an explicit no-regression constraint. Their order is
        # fixed; no easy terrain can compensate for a worse difficult family.
        for family in families:
            indices = [i for i, episode in enumerate(episodes) if episode["terrain_family"] == family]
            metrics.append(
                {
                    "name": f"{name}:{family}",
                    "values": [
                        1.0
                        if name in ("stopping", "turning")
                        and not measured[i]["stop_exposed" if name == "stopping" else "turn_exposed"]
                        else measured[i][name]
                        for i in indices
                    ],
                    "sample_keys": [str(episodes[i]["episode_seed"]) for i in indices],
                    "direction": "higher",
                    "margin": rules[margin_key],
                }
            )
    return {
        "protocol": {
            "version": rules["version"],
            "episode_seeds": report["protocol"]["episode_seeds"],
            "environment_run_seed": report["protocol"]["environment_run_seed"],
            "families": families,
            "horizon_steps": report["protocol"]["horizon_steps"],
            "dt": report["protocol"]["dt"],
            "task_sha256": _digest(identity),
...
def certify_and_publish_behavior(
    *,
    output: Path,
    library: Path,
    recipe_path: Path,
    ...
    from environments.shared.certified_library import NoRecommendedCandidate, copy_recommended, publish_candidate
    from environments.shared.train_behaviors import _verify_bundle

    rules = load_certification_rules()
    probe = _panel_env(species, recipe_path, int(rules["certification_run_seed"]))
    try:
        families = probe.terrain_families
    finally:
        probe.close()
    if (
        isinstance(comparison_episodes, bool)
        or not isinstance(comparison_episodes, int)
        or comparison_episodes < 2 * len(families)
    ):
        raise ValueError(f"Comparison requires at least {2 * len(families)} episodes (two per terrain family)")
    model_path, norm_path = output / "model.zip", output / "vecnormalize.pkl"
    _verify_bundle(model_path, norm_path, dict(recipe))
    original_hashes = sha256_file(model_path), sha256_file(norm_path)
    run = json.loads((output / "run.json").read_text())
    parent = run.get("certification_training_parent_sha256")
    parent_norm = run.get("certification_training_parent_normalization_sha256")
    if any(not isinstance(value, str) or not value.startswith("sha256:") for value in (parent, parent_norm)):
        raise ValueError("Certification requires the recorded training-stage parent model and normalization hashes")
    report = evaluate_saved_panel(
        model_path=model_path,
        normalization_path=norm_path,
        recipe_path=recipe_path,
        species=species,
        identity=identity,
        output_dir=output / "certification" / "evaluation",
        run_seed=int(rules["certification_run_seed"]),
        seed_start=int(rules["certification_seed_start"]),
        episodes=int(rules["episodes_per_family"]) * len(families),
    )
    certificate = judge_behavior_panel(report, identity, rules)
    lineage = report.get("training_lineage", {})
    start, end = lineage.get("stage_start_timesteps"), lineage.get("num_timesteps")
    update_start, update_end = lineage.get("stage_start_updates"), lineage.get("optimizer_updates")
    if (
        lineage.get("seed") != training_seed
        or lineage.get("parent_model_sha256") != parent
        or lineage.get("parent_normalization_sha256") != parent_norm
        or isinstance(start, bool)
        or isinstance(end, bool)
        or not isinstance(start, int)
        or not isinstance(end, int)
        or end <= start
        or isinstance(update_start, bool)
        or isinstance(update_end, bool)
        or not isinstance(update_start, int)
        or not isinstance(update_end, int)
        or update_end <= update_start
    ):
        certificate["passed"] = False
        certificate["failures"].append(
            "Saved model must prove its original training lineage and completed optimization steps"
        )
    certificate["training_lineage"] = lineage
    certificate.update(
        model_sha256=sha256_file(model_path),
        normalization_sha256=sha256_file(norm_path),
        evidence_sha256=sha256_file(output / "certification/evaluation/evaluation_summary.json"),
    )
    gate_path = output / "certification" / "certificate.json"
    gate_path.write_text(json.dumps(certificate, indent=2, allow_nan=False) + "\n")
    metadata = recipe.get("behavior", recipe.get("pilot", {}))
    behavior = str(metadata["name"])
    ...
    if certificate["passed"]:
        panel = evaluate_saved_panel(
            ...
            species=species,
            identity=identity,
            output_dir=output / "comparison" / "candidate",
            run_seed=int(rules["comparison_run_seed"]),
            seed_start=int(rules["comparison_seed_start"]),
            episodes=comparison_episodes,
        )
        comparison = comparison_from_panel(panel, identity)
        try:
            incumbent = copy_recommended(library, key, output)
        except NoRecommendedCandidate:
            incumbent = None
        if incumbent is not None and (incumbent.get("comparison") or {}).get("protocol") != comparison["protocol"]:
            panel = evaluate_saved_panel(
                model_path=Path(incumbent["model"]),
                normalization_path=Path(incumbent["normalizer"]),
                recipe_path=recipe_path,
                species=species,
                identity=identity,
                output_dir=output / "comparison" / "incumbent",
                run_seed=int(rules["comparison_run_seed"]),
                seed_start=int(rules["comparison_seed_start"]),
                episodes=comparison_episodes,
            )
            incumbent_comparison = comparison_from_panel(panel, identity)
        (output / "comparison" / "candidate.json").write_text(json.dumps(comparison, indent=2) + "\n")
    # The run retains complete certified input snapshots. The library stores
    # this model's artifacts and ancestry hashes without recursively duplicating
    # previous library versions or periodic optimizer snapshots.
    files = {
        path.relative_to(output).as_posix(): path
        for path in output.rglob("*")
        if path.is_file() and not {"checkpoints", "certified_inputs"}.intersection(path.relative_to(output).parts)
    }
    if original_hashes != (sha256_file(model_path), sha256_file(norm_path)):
        raise ValueError("Candidate pair changed during certification and comparison")
    publication = publish_candidate(
        library,
        key=key,
        recipe_sha256=_digest(
            {"recipe": recipe, "parent_checkpoint_sha256": parent, "parent_normalization_sha256": parent_norm}
        ),
        ...
        required_seeds=int(rules["certification_seeds"]),
        metadata={
            "kind": "behavior",
