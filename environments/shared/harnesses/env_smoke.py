...
def check_basic_functionality(env_class, render: bool = False):
...
def check_episode_rollout(env_class, num_episodes: int = 3, max_steps: int = 100, render: bool = False):
...
def check_reward_components(env_class, reward_keys: list[str], render: bool = False):
...
def check_determinism(env_class):
...
def check_observation_bounds(env_class):
...
def run_env_tests(env_class, reward_keys: list[str], description: str) -> int:
    ...
    all_passed = True
    all_passed &= check_basic_functionality(env_class, render=args.render)
    all_passed &= check_episode_rollout(env_class, args.episodes, args.steps, render=args.render)
    all_passed &= check_reward_components(env_class, reward_keys, render=args.render)
    all_passed &= check_determinism(env_class)
    all_passed &= check_observation_bounds(env_class)
