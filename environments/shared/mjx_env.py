...
class MJXEnvConfig:
    ...
    # §3.3).  0.0 disables — the step/reset kernels bake the legacy
    # push-free trace, so trajectories are byte-identical to the
    # pre-perturbation code.  Task-level and stage-tunable (deliberately
    # NOT part of the versioned plant interface: a push changes the task,
    # not what a checkpoint's actions mean; the task fingerprint, not the
    # policy interface, distinguishes pushed runs).  Semantics are shared
    # with BaseDinoEnv via environments/shared/perturbation.py.
...
class EnvState:
    ...
    # Scheduled-push carry (perturbation_* config).  Fixed for the episode,
    # regenerated by (auto-)reset; empty (0,)-shaped when pushes are off so
...
class MJXDinoEnv:
    ...
    def __init__(self, species: str, stage: int = 1, num_envs: int = 2048, env_kwargs: dict[str, Any] | None = None):
        ...
        # forcing a re-trace.
        # Scheduled-push constants (perturbation_* config), derived from the
        # host model once and closed over by the step/reset builders below.
        # None means off: both kernels bake the push-free trace.
        self._push_constants: "dict[str, Any] | None" = None
        if self.config.perturbation_capture_velocity_multiple > 0.0:
            from .perturbation import derive_push_parameters, max_pushes_for, validate_push_config

            control_dt = float(self.mj_model.opt.timestep) * self.config.frame_skip
            schedule_steps = validate_push_config(
                capture_velocity_multiple=self.config.perturbation_capture_velocity_multiple,
                interval_s=self.config.perturbation_interval,
                jitter_s=self.config.perturbation_jitter,
    ...
    def _make_step_fn(self):
        ...
        # Trace-time gate: None bakes the push-free kernel.
        push_constants = self._push_constants
        ...
        def step_fn(state: EnvState, action, rng, forward_vel_scale):
            ...
            if push_constants is not None:
                # Scheduled external push, in lockstep with BaseDinoEnv.step:
                # the root row is rewritten every control step and the kernel
                # returns zero outside every window, so the force self-clears
                # and can never persist past its schedule (or a reset — the
                # reset state's xfrc is the zeroed default).  Reward and
                # observation are deliberately untouched.
                from .perturbation import external_push_force

                push_force = external_push_force(
                    state.step_count,
                    state.push_start_steps,
                    state.push_directions,
                    duration_steps=push_constants["duration_steps"],
                    force_newtons=push_constants["force_n"],
                )
                data = data.replace(
                    xfrc_applied=data.xfrc_applied.at[push_constants["root_body_id"], 0:3].set(push_force)
                )
    ...
    def _make_reset_fn(self):
        ...
        def reset_fn(rng):
            ...
            if push_constants is not None:
                # New episode, new push schedule.  The seed comes from a
                # fold_in of the reset key, so the existing draw sequence
                # above is untouched and a push-free reset of the same key
                # is bit-identical.  Determinism holds per backend: the
                # same reset key always yields the same schedule (the PRF
                # itself is bit-identical across backends; only the
                # key-to-seed draw differs from SB3's np_random draw).
                from .perturbation import push_schedule

                schedule_seed = jax.random.randint(
                    jax.random.fold_in(rng, 0x9E3779B9),
                    (),
                    0,
                    jnp.iinfo(jnp.int32).max,
                    dtype=jnp.int32,
                ).astype(jnp.uint32)
                push_start_steps, push_directions = push_schedule(
                    schedule_seed,
                    max_pushes=push_constants["max_pushes"],
                    interval_steps=push_constants["interval_steps"],
                    jitter_steps=push_constants["jitter_steps"],
                )
            else:
                push_start_steps = jnp.zeros((0,), dtype=jnp.int32)
                push_directions = jnp.zeros((0, 2), dtype=jnp.float32)
    ...
    def perturbation_manifest(self) -> "dict[str, Any] | None":
        """Per-species derived push parameters, for run provenance.

        ``None`` while perturbation is off.  Mirrors
        ``BaseDinoEnv.perturbation_manifest`` — both backends derive from
        the same host model, so the derived constants are identical.
        """
        if self._push_constants is None:
            return None
        manifest: dict[str, Any] = dict(self._push_constants["params"])
        manifest["interval_s"] = self.config.perturbation_interval
