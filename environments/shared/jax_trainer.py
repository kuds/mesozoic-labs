...
Extracts the main training loop from the Colab notebook into a reusable
module.  The notebook reduces to: configure, call ``train()``, plot results.

Usage::

    from environments.shared.jax_trainer import TrainConfig, TrainResult, train

    config = TrainConfig(num_envs=2048, rollout_len=64, ...)
    result = train(config, env, network, params, obs_rms, reward_cfg)

    # result.params, result.reward_history, etc. available for plotting
"""
