...
def validate_behavior_selection(
    *,
    ...
    if not (checkpoint.strip() and vecnormalize.strip()):
...
def build_notebook_behavior_plan(
    *,
    ...
    model = source_path(checkpoint)
    normalizer = source_path(vecnormalize)
    ...
    return NotebookBehaviorPlan(
        ...
        vecnormalize_path=normalizer,
        output_dir=output,
        load_mode=load_mode,
        seed=actual_seed,
        steps=steps if steps is not None else 4096 if quick_test and not eval_only else None,
        eval_only=eval_only,
...
def display_notebook_behavior(output_dir: Path) -> None:
    ...
    print("Saved diagnostics only: a behavior run is not a canonical certification.")
